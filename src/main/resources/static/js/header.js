...
async function getAlarm() {
	if (checkToken() != null) {
		...
		const alarm_container = document.querySelector("#alarm-container");
		for (let i = 0; i < alarm_items_info.length; i++) {
			const addElement = document.createElement("div");
			addElement.setAttribute("class", "alarm-item");
			addElement.innerHTML =
				alarm_items_info[i].notifyType == 0 ? alarm_container_comment(alarm_items_info[i]) : alarm_container_like(alarm_items_info[i]);
			addElement.innerHTML = alarm_container(alarm_items_info[i]);
			alarm_container.addElement(addElement);
		}
	} else {
		const addElement = document.createElement("div");
		addElement.setAttribute("class", "alarm-item");
